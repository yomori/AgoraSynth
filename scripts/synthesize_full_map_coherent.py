"""Generate a coherent full-footprint Compton-y map via Repaint-style stitching.

Unlike ``synthesize_full_map.py`` (independent patches blended with a cosine
window), this script enforces consistency across patch boundaries: when
generating a patch that overlaps an already-generated region, the overlap
pixels are constrained to track the linear-interpolation path during ODE
integration. The model fills in the rest of the patch consistently with
that boundary, so adjacent patches agree at the overlap by construction.

Patch scheduling
----------------
Patches placed on a regular grid with stride ``s = (1 - overlap) * patch``
are 4-colored: ``(i % 2, j % 2)`` -> phase 0..3. For overlap <= 0.5, no two
patches in the same phase touch each other, so we can sample an entire
phase as a single batched ODE call. We sweep phase 0 unconditionally,
then phases 1, 2, 3 conditioned on whatever was already generated.

The global state is held in *gaussianized* log-y space (the model's
domain). At the end we de-gaussianize once and write physical y as FITS.

Caveats
-------
- Overlap > 0.5 needs a finer coloring (more phases / smaller batches);
  not implemented. The default 0.25 overlap is fine.
- Each patch is placed as a 188x188 ZEA tangent-plane block at a fixed
  pixel offset in the *single global ZEA*. Edge patches at large angles
  from the global tangent point are mildly distorted vs. training (~10-15%
  pixel-scale change at 50 deg from tangent for SPT-1500).
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--out", type=Path, default=Path("samples/full_map_coherent.fits"))
    ...
    print(
        f"  global ZEA tangent at ({ra_c:.2f}, {dec_c:.2f}); grid "
        f"{n_pix_x} x {n_pix_y} pixels"
    )
    ...
    print(
        f"  patches: {len(xs)} x {len(ys)} = {n_total} total | "
        f"phases: {[len(p) for p in phases]} (sum={n_total})"
    )
    ...
    for phase_idx, phase_positions in enumerate(phases):
        ...
        for batch_start in range(0, len(phase_positions), args.batch_size):
            ...
            if phase_idx == 0:
                # Phase 0 patches don't overlap each other and have no
                # pre-existing constraint -- pure unconditional sampling.
                x_batch = sample_heun(
                    apply_fn, params,
            ...
            for k, (px, py) in enumerate(chunk):
                ...
                # Already-filled pixels keep their value (the model produces
                # them via the constraint anyway, but enforce exactly to
                # eliminate any small numerical drift from the ODE).
            ...
            n_done = (
                sum(len(p) for p in phases[:phase_idx])
                + batch_start + B
            )
            print(
                f"    phase {phase_idx}/3  {n_done}/{n_total} patches  "
    ...
    if not filled.all():
        ...
        print(f"  WARNING: {n_uncov} pixels left uncovered (filling with zeros).")

    # ------------------------------------------------------------------
    # De-gaussianize and save
    # ------------------------------------------------------------------
    print("de-gaussianizing to physical y ...")
    global_y_full = np.asarray(gaussianized_to_physical(
        jnp.asarray(global_x[..., 0]), quantile_grid, z_grid, y0=y0
    ))
    print(
        f"  final map: {global_y_full.shape}, y range "
        f"[{global_y_full.min():.3e}, {global_y_full.max():.3e}]"
    )

    args.out.parent.mkdir(parents=True, exist_ok=True)
    primary = fits.PrimaryHDU(
        data=global_y_full.astype(np.float32), header=wcs.to_header()
    )
    ...
    fits.HDUList([primary]).writeto(args.out, overwrite=True)
